...
This repo is **not** an application — it is the `/onboard` **Claude Code Skill** itself (current version: v2.11.1). The "code" being maintained is:
...
- `tests/run.sh` + `tests/integration/*.sh` — in-repo sandbox tests for plugin install / hook execution / installer round-trip / SKILL.md link consistency / hook runtime behavior / v2.11 uninstall modes (150 assertions across 9 integration tests: `plugin-install.sh` 24 + `hook-mirror.sh` 29 + `install-roundtrip.sh` 34 + `skillmd-links.sh` 9 + `hook-behavior.sh` 22 + `cc-plugin-detection.sh` 8 + `prepare-script-detection.sh` 2 + `sync-versions-detection.sh` 1 + `uninstall-modes.sh` 21); run via `bash tests/run.sh`. v2.11.1 adds `scripts/verify-counts.sh` to auto-check the headline+per-test claims here / in CHANGELOG against actual `pass:` counts — drift triggers non-zero exit. Persisted post-v2.10 from the prior `/tmp/onboard-plugin-test.sh`; `hook-behavior.sh` added in v2.10.2; `cc-plugin-detection.sh` / `prepare-script-detection.sh` / `sync-versions-detection.sh` / `uninstall-modes.sh` added in v2.11.0 for the 8 P-A items.
